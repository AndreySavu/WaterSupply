...
class TkinterMapView(tkinter.Frame):
    def __init__(self, *args,
        ...
        self.right_click_menu_commands_markers : List[dict] = [] 
        self.right_click_menu_commands_lines : List[dict] = [] 
    ...
    def save_state_to_file(self):
        print("penis1")
        data = {"markers": self.canvas_marker_list
                }
        print(data)
    ...
    def add_right_click_menu_command_markers(self, label: str, command: Callable, pass_coords: bool = False) -> None:
        self.right_click_menu_commands_markers.append({"label": label, "command": command, "pass_coords": pass_coords})

    def add_right_click_menu_command_lines(self, label: str, command: Callable, pass_coords: bool = False) -> None:
        self.right_click_menu_commands_lines.append({"label": label, "command": command, "pass_coords": pass_coords})
    ...
    def mouse_right_click(self, event):
        ...
        for marker in self.canvas_marker_list:
            if not (marker.get_canvas_pos(coordinate_mouse_pos)[0] > marker.get_canvas_pos(marker.position)[0] + 20 or
                marker.get_canvas_pos(coordinate_mouse_pos)[0] < marker.get_canvas_pos(marker.position)[0] - 20 or
                marker.get_canvas_pos(coordinate_mouse_pos)[1] > marker.get_canvas_pos(marker.position)[1] + 20 or
                marker.get_canvas_pos(coordinate_mouse_pos)[1] < marker.get_canvas_pos(marker.position)[1] - 20): 
                object_name = marker.data
                for command in self.right_click_menu_commands_markers:
                    if command["pass_coords"]:
                        m.add_command(label=command["label"], command=partial(command["command"], coordinate_mouse_pos))
                    else:
                        m.add_command(label=command["label"], command=partial(command["command"], object_name))
                flag = 1
                break
        if not flag and len(self.canvas_polygon_list) != 0:
            for poly in self.canvas_polygon_list:
                a = poly.get_canvas_pos(poly.position_list[0])
                b = poly.get_canvas_pos(poly.position_list[1])
                c = poly.get_canvas_pos(coordinate_mouse_pos)

                if abs(math.sqrt((a[0] - b[0])**2 + (a[1] - b[1])**2) - 
                    (math.sqrt((a[0] - c[0])**2 + (a[1] - c[1])**2) + math.sqrt((b[0] - c[0])**2 + (b[1] - c[1])**2))) < 0.4:
                    object_name = poly.data
    ...
    def set_marker(self, deg_x: float, deg_y: float, text: str = None, icon = "default.png", **kwargs) -> CanvasPositionMarker:
        marker = CanvasPositionMarker(self,position= (deg_x, deg_y), text= text, icon= tkinter.PhotoImage(file = icon), **kwargs)
        marker.draw()
