...
class App:
    ...
    def add_marker_source(self, coords):
        ...
        dat = "Source" + str(self.amount_of_objects[0] + 1)
        self.map_widget.set_marker(coords[0], coords[1],
                                    text = "ИСТ " + adress,
                                    icon = "icons/source.png",
                                    data = dat)                          
        self.gr.add_vertex(dat, coords[0], coords[1], (0, 0, 0, 0))
        self.amount_of_objects[0] += 1

    def add_marker_tower(self, coords):
        adress = str(TKmv.convert_coordinates_to_address(coords[0], coords[1]).street) + \
            " " + str(TKmv.convert_coordinates_to_address(coords[0], coords[1]).housenumber)
        dat = "WaterTower" + str(self.amount_of_objects[1] + 1)
        self.map_widget.set_marker(coords[0], coords[1],
                                                text = "ВНБ " + adress,
                                                icon = "icons/tower.png",
                                                data = dat)                          
        self.gr.add_vertex(dat, coords[0], coords[1], (0, 0, 0, 0, 0))
        self.amount_of_objects[1] += 1
        
    def add_marker_reservoir(self, coords):
        adress = str(TKmv.convert_coordinates_to_address(coords[0], coords[1]).street) + \
            " " + str(TKmv.convert_coordinates_to_address(coords[0], coords[1]).housenumber)
        dat = "CounterReservoir" + str(self.amount_of_objects[2] + 1)
        self.map_widget.set_marker(coords[0], coords[1],
                                    text = "КР " + adress,
                                    icon = "icons/reservoir.png",
                                    data = dat)                          
        self.gr.add_vertex("CounterReservoir" + str(len(self.gr.get_all_vertexes())), coords[0], coords[1], (0, 0, 0, 0))
        self.amount_of_objects[2] += 1

    def add_marker_consumer(self, coords):
        adress = str(TKmv.convert_coordinates_to_address(coords[0], coords[1]).street) + \
            " " + str(TKmv.convert_coordinates_to_address(coords[0], coords[1]).housenumber)
        dat = "Consumer" + str(self.amount_of_objects[3] + 1)
        self.map_widget.set_marker(coords[0], coords[1],
                                    text = "ПТР " + adress,
                                    icon = "icons/consumer.png",
                                    data = dat)
        self.gr.add_vertex(dat, coords[0], coords[1], (0, 0, 0, 0))
        self.amount_of_objects[3] += 1

    def add_marker_connector(self, coords):
        dat = "Connector" + str(self.amount_of_objects[4] + 1)
        self.map_widget.set_marker(coords[0], coords[1],
                                    text = "РЗВ " + str(round(coords[0],4)) + "; " + str(round(coords[1],4)),
                                    icon = "icons/connector.png",
                                    data = dat)
        self.gr.add_vertex(dat, coords[0], coords[1], (0, 0, 0, 0))
        self.amount_of_objects[4] += 1
    ...
    def open_new_map(self, pos=(55.010159, 82.925716), zoom = 10):
        self.gr.clear_graph()
        self.close_properties()
        self.amount_of_objects = [0, 0, 0, 0, 0, 0]
    ...
    def save(self):
        ...
        if self.path_to_file =='':
            ...
            cu.execute("INSERT INTO map (pos1, pos2, zoom) VALUES (?, ?, ?);", (pos[0], pos[1], self.map_widget.zoom))

            for item in self.gr.get_all_vertexes():
                if item[0][:4] == "Sour":
                    print("src")
                    cu.execute("INSERT INTO sources (name, coord1, coord2, H, G, Gmax, P) VALUES (?, ?, ?, ?, ?, ?, ?);", 
                            (item[0], float(item[1]), float(item[2]), float(item[3][0]), 
                                        float(item[3][1]), float(item[3][2]), float(item[3][3])))     
                if item[0][:4] == "Wate":
                    print("twr")
                    cu.execute("INSERT INTO towers (name, coord1, coord2, H, G, Hwater, V, P) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", 
                            (item[0], float(item[1]), float(item[2]), float(item[3][0]), 
                                        float(item[3][1]), float(item[3][2]), float(item[3][3]), float(item[3][4])))  
                if item[0][:4] == "Coun":
                    print("res")
                    cu.execute("INSERT INTO reservoirs (name, coord1, coord2, H, G, Hwater, P) VALUES (?, ?, ?, ?, ?, ?, ?);", 
                            (item[0], float(item[1]), float(item[2]), float(item[3][0]), 
                                        float(item[3][1]), float(item[3][2]), float(item[3][3])))   
                if item[0][:4] == "Conn":
                    print("cnn")
                    cu.execute("INSERT INTO connectors (name, coord1, coord2, H, G, P, Pressure) VALUES (?, ?, ?, ?, ?, ?, ?);", 
                            (item[0], float(item[1]), float(item[2]), float(item[3][0]), 
                                        float(item[3][1]), float(item[3][2]), float(item[3][3])))  
                if item[0][:4] == "Cons":
                    print("cns")
                    cu.execute("INSERT INTO consumers (name, coord1, coord2, H, G, Pmin, P) VALUES (?, ?, ?, ?, ?, ?, ?);", 
                            (item[0], float(item[1]), float(item[2]), float(item[3][0]), 
                                        float(item[3][1]), float(item[3][2]), float(item[3][3])))  
            for item in self.gr.get_all_edges():
                print("pip")
                cu.execute("INSERT INTO pipes (name, name1, name2, H1, H2, D, Len, G, R, Glost, Speed, Lambda, Lost,\
